...
func TestRequestMsg(t *testing.T) {
	...
	go func() {
		for {
			...
			case m := <-c:
				t.Log(m.Header, string(m.Data))
				err := m.Respond([]byte("recv:" + string(m.Data)))
				require.NoError(t, err)
			}
		}
	}()
	// Subscribe
	if _, err := nc.Subscribe(subName, func(m *nats.Msg) {
		c <- m
	}); err != nil {
		log.Fatal(err)
	}
	...
	for i := 0; i < 10; i++ {
		go func() {
			t.Log("req")
			req := nats.NewMsg(subName)
			req.Header.Set("ser_name", "self name")
			req.Header.Set("ser_id", "self_id")
			req.Data = []byte("hello world")
			msg, err := nc.RequestMsg(req, time.Second*3)
			require.NoError(t, err)
			t.Log("ack", string(msg.Data))
		}()
	}
...
func TestDiffSub(t *testing.T) {
	...
	if _, err := nc.Subscribe(subName1, func(m *nats.Msg) {
		d, _ := strconv.Atoi(string(m.Data))
		t.Log("recv", d)
		c <- d
	}); err != nil {
		log.Fatal(err)
	}
	if _, err := nc.Subscribe(subName2, func(m *nats.Msg) {
		d, _ := strconv.Atoi(string(m.Data))
		t.Log("recv", d)
		c <- d
	}); err != nil {
	...
	for i := 0; i < 100; {
		nc.Publish(subName1, []byte(strconv.Itoa(i)))
		t.Log("publish", i)
		nc.Publish(subName2, []byte(strconv.Itoa(i+1)))
		t.Log("publish", i+1)
		i += 2
	}
